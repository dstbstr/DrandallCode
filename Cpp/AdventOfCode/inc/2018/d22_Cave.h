...
auto PartOne(RowCol target, u32 depth) {
...
auto PartTwo(RowCol target, u32 depth) {
...
std::string Run(const std::vector<std::string>&) {
    ...
    //return ToString(PartOne(target, 7863));
    return ToString(PartTwo(target, 7863));
}

bool RunTests() {
    RowCol target = { 10, 10 };
    //if (PartOne(target, 510) != 114) return false;
    if (PartTwo(target, 510) != 45) return false;
    return true;
}
//depth: 7863
//target: 14, 760