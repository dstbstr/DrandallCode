...
Bp ParseLine(const std::string& line) {
    ...
    ParseNumber(match[1].str(), bp.Index);
    ParseNumber(match[2].str(), bp.Costs[Ore].Costs[Ore]);
    ParseNumber(match[3].str(), bp.Costs[Clay].Costs[Ore]);
    ParseNumber(match[4].str(), bp.Costs[Obsidian].Costs[Ore]);
    ParseNumber(match[5].str(), bp.Costs[Obsidian].Costs[Clay]);
    ParseNumber(match[6].str(), bp.Costs[Geode].Costs[Ore]);
    ParseNumber(match[7].str(), bp.Costs[Geode].Costs[Obsidian]);
...
std::string Run(const std::vector<std::string>& lines) {
    //return ToString(SumQualityLevels(lines));
    return ToString(MultiplyQualityLevels(lines));
}
