...
constexpr std::array<std::array<char, Cols>, Rows> BuildMap(const std::vector<std::string>& lines, size_t& startIndex, size_t& endIndex) {
    ...
    for (size_t row = 0; row < Rows; row++) {
        for (size_t col = 0; col < Cols; col++) {
            ...
            if (lines[row][col] == 'S') {
                result[row][col] = 'a';
                startIndex = GetIndex<Rows, Cols>(row, col);
            }
            else if (lines[row][col] == 'E') {
                result[row][col] = 'z';
                endIndex = GetIndex<Rows, Cols>(row, col);
            }
        }
    }
...
constexpr std::vector<size_t> FindAllStarts(const std::array<std::array<char, Cols>, Rows>& map) {
    ...
    for (size_t row = 0; row < Rows; row++) {
        for (size_t col = 0; col < Cols; col++) {
            if (map[row][col] == 'a') {
                result.push_back(GetIndex<Rows, Cols>(row, col));
            }
        }
    }
...
constexpr std::vector<size_t> FindNext(size_t currentIndex) {
    size_t row, col;
    GetRowCol<Rows, Cols>(currentIndex, row, col);

    std::vector<size_t> result{};
    if (row > 0) {
        result.push_back(GetIndex<Rows, Cols>(row - 1, col));
    }
    if (col > 0) {
        result.push_back(GetIndex<Rows, Cols>(row, col - 1));
    }
    if (row < Rows - 1) {
        result.push_back(GetIndex<Rows, Cols>(row + 1, col));
    }
...
constexpr u32 Bfs(const std::array<std::array<char, Cols>, Rows>& map, size_t startIndex, size_t endIndex) {
    ...
    size_t row, col;
    GetRowCol<Rows, Cols>(startIndex, row, col);
    ...
    while (true) {
        for (auto index : current) {
            ...
            for (auto move : moves) {
                GetRowCol<Rows, Cols>(move, row, col);
...
std::string RunFirst(const std::vector<std::string>& lines) {
    ...
    return ToString(Bfs<41, 66>(map, startIndex, endIndex));
}
...
bool RunTests() {
    ...
    size_t row, col;
    GetRowCol<5, 8>(startIndex, row, col);
    if (map[row][col] != 'a') return false;

    GetRowCol<5, 8>(endIndex, row, col);
