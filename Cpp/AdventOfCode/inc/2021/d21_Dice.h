...
constexpr auto PartOne(size_t p1Pos, size_t p2Pos) {
...
static_assert(PartOne(4, 8) == 739785);
static_assert(PartOne(6, 10) == 853776);
...
auto PartTwo(size_t p1Pos, size_t p2Pos) {
    p1Pos--;
    p2Pos--;
...
std::string Run(const std::vector<std::string>&) {
    //return ToString(PartOne(6, 10));
    return ToString(PartTwo(6, 10));
}

bool RunTests() {
    if (PartOne(4, 8) != 739785) return false;
    if (PartTwo(4, 8) != 444356092776315u) return false;
    return true;
}