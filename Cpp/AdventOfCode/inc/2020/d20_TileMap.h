...
constexpr std::vector<bool> GetEdge(const PixelGrid& pixels, Direction dir) {
    switch (dir) {
...
constexpr Direction GetOtherEdge(Direction dir) {
    switch (dir) {
    case Down: return Up;
    case Up: return Down;
...
constexpr bool DoEdgesMatch(const Tile& t1, const Tile& t2, Direction e1, Direction e2) {
...
constexpr bool DoEdgesMatch(const Tile& t1, const Tile& t2, Direction edge) {
            ...
            for (size_t i = 0; i < lTile.EdgeCounts.size(); i++) {
                for (size_t j = 0; j < rTile.EdgeCounts.size(); j++) {
                    if (lTile.EdgeCounts[i] == rTile.EdgeCounts[j]) {
                        auto d1 = static_cast<Direction>(i);
                        auto d2 = static_cast<Direction>(j);
...
constexpr bool DoEdgesMatchExact(const Tile& t1, const Tile& t2, Direction edge) {
    Direction otherEdge = edge;
    switch (edge) {
...
constexpr bool OrientPiece(const Tile& fixedPiece, Tile& toFit, Direction edge) {
    bool found = false;
    for (auto i = 0; i < 4; i++) {
