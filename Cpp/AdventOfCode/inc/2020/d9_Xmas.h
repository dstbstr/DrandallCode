...
auto PartOne(const std::vector<std::string>& lines, size_t windowSize) {
...
auto PartTwo(const std::vector<std::string>& lines, size_t windowSize) {
    const auto nums = ParseInput<size_t>(lines);
...
std::string Run(const std::vector<std::string>& lines) {
    //return ToString(PartOne(lines, 25));
    return ToString(PartTwo(lines, 25));
}
...
bool RunTests() {
    ...
    if (PartOne(lines, 5) != 127) return false;
    if (PartTwo(lines, 5) != 62) return false;
    return true;
}