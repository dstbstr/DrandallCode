...
u32 CountCombinations(const std::vector<u32>& containers, size_t index, u32 remaining) {
...
u32 CountCombinations(std::vector<u32>& containerSizes, u32 initial) {
...
u32 GetMinSize(const std::vector<u32>& containers, size_t index, u32 remaining, u32 currentLength) {
    auto next = containers[index];
    if(remaining < next) {
        return 0;
    }
...
u32 CountCombinationsWithMaxContainers(const std::vector<u32>& containers, size_t index, u32 remaining, u32 currentLength, const u32 maxSize) {
...
u32 CountSmallContainers(std::vector<u32>& containers, u32 initial) {
...
bool RunTests() {
    ...
    ASSERT(CountCombinations(containers, 25) == 4);
    ASSERT(GetMinSize(containers, 0, 25, 0) == 3); //0 10 15
    ASSERT(CountSmallContainers(containers, 25) == 3);
    return true;
}
#undef ASSERT