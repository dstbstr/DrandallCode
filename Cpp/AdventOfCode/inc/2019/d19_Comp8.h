...
bool IsInBeam(const std::vector<s64>& instructions, UCoord pos) {
...
bool CanContainBox(const std::vector<s64>& instructions, u32 row, u32 col, u32 boxSize) {
    u32 bottomLeft = col;
...
UCoord FindRoughPosition(const std::vector<s64>& instructions, u32 boxSize) {
    u32 row = boxSize;
    u32 col = boxSize / 2;
...
UCoord FineTune(const std::vector<s64>& instructions, const UCoord& pos, u32 boxSize) {
    for (u32 row = pos.Y; row < pos.Y + boxSize; row++) {
        for (u32 col = pos.X; col < pos.X + boxSize; col++) {
...
bool Validate(const std::vector<s64>& instructions, const UCoord& pos, u32 boxSize) {
