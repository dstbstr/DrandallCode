...
//bright orange bags contain no other bags.
    using Map = std::unordered_map<std::string, std::unordered_map<std::string, size_t>>;

    Map BuildMap(const std::vector<std::string>&lines) {
        Map result;
        for (const auto& line : lines) {
            ...
            std::unordered_map<std::string, size_t> entry;
    ...
    //if(bagCount.contains(innerKey)) bagCount[key] += bagCount[innerKey] * quantity
    //else if(map[innerKey].empty()) return 1
    //else recurse

    using BagCache = std::unordered_map<std::string, size_t>;
    ...
    bool RunTests() {
        ...
        if (Part2(lines) != 32) return false;
        ...
        if (Part2(lines) != 126) return false;
