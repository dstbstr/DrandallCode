...
SOLUTION(2020, 24) {
    ...
    using Grid = std::unordered_set<Vec3<s64>>;
    ...
    auto Part2(const std::vector<std::string>&lines, size_t days) {
        auto grid = GetInitialGrid(lines);
    ...
    std::string Run(const std::vector<std::string>&lines) {
        ...
        return Constexpr::ToString(Part2(lines, 100));
    }
    ...
    bool RunTests() {
        ...
        if (Part1(lines) != 10) return false;
        if (Part2(lines, 1) != 15) return false;
        if (Part2(lines, 100) != 2208) return false;
        return true;
    }
